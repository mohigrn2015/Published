...
$(document).ready(function () {
    ...
    if (window.location.href == basedURL +'ServiceDetails/Create') {
    ...
    if (window.location.href == basedURL + 'ServiceDetails/Create') {
        window.onbeforeunload = function () {
            if (window.location.href == basedURL + 'ServiceDetails/Create' && $("#is_break").val() == '0') {
                if ($("#start_time").val() != '' || sessionStorage.getItem("service") == 1) { 
...
function NewServiceNo() {
    ...
    if ($("#is_break").val() == 1) {
        modalConfirm("Do you want to Take a Break?", breakcall, Recall); //marked
    }
    else {
        if (window.location.href == basedURL + 'ServiceDetails/Create') {
            Recall();
...
window.onerror = function (message, source, lineno, colno, error) {
    ...
    var errorLoggerObj = {
        ...
        "message": this.getBrowser() + '|| Source: ' + source + ' || line no: ' + lineno + ' || column:' + colno + '||  ' + message + ' || Error: ' + error,
        "controllerName": '',
        "actionName": ''
    };
